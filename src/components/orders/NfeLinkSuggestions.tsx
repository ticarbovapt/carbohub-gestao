...
function SuggestionRow({ s, dismiss }: { s: NfeLinkSuggestion; dismiss: (id: string) => void }) {
  const linkMutation = useLinkNFeToOrder();
  const conf = CONF_STYLE[s.confidence];

  const handleConfirm = () => {
    linkMutation.mutate(
      { nfeId: s.nfeId, orderNumber: s.orderNumber },
      { onSuccess: () => dismiss(s.orderId) },
    );
  };
  ...
  return (
    <div className="rounded-lg border border-border bg-background/40 p-3 space-y-2">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <span className={`text-[10px] px-2 py-0.5 rounded-full border font-medium ${conf.cls}`}>
          {conf.label}
        </span>
        <div className="flex items-center gap-1.5 flex-wrap">
          {s.reasons.map((r, i) => (
            <span key={i} className="text-[10px] text-muted-foreground bg-muted/50 rounded px-1.5 py-0.5">
              {r}
            </span>
          ))}
        </div>
      ...
      <div className="flex items-center gap-3 text-sm flex-wrap">
        ...
        <div className="min-w-0">
          ...
          <p className="text-[11px] text-muted-foreground">
            {fmtBRL(s.orderTotal)} · {fmtDate(s.orderDate)}
          </p>
        </div>
        ...
        <div className="min-w-0">
          ...
          <p className="text-[11px] text-muted-foreground">
            {fmtBRL(s.nfeValor)} · {fmtDate(s.nfeData)}
          </p>
        </div>
      ...
      <CarboCardContent className="p-0">
        ...
        >
          ...
          <span className="font-semibold text-sm">
            Sugestões de vínculo NF ↔ pedido
          </span>
        ...
        {open && (
          <div className="px-4 pb-4 space-y-2">
            <p className="text-[11px] text-muted-foreground flex items-center gap-1.5">
              ...
              O sistema recomenda com base em cliente, valor e data. Confira antes de confirmar — o vínculo só é aplicado quando você clica.
            </p>
            {visible.map(s => (
              <SuggestionRow key={s.orderId} s={s} dismiss={dismiss} />
            ))}
          </div>
        )}
      </CarboCardContent>
