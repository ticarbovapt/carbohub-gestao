...
interface CanalMetasDialogProps {
  open: boolean;
...
const CANAIS = [
  { key: "consumo", label: "Consumo (B2B)", hint: "deixe vazio p/ usar a regra automática (mês anterior +15%)" },
  { key: "revenda", label: "Revenda (PDV)", hint: "Nordeste + Sudeste" },
  { key: "online",  label: "On-line",       hint: "a partir de jul/26" },
] as const;

export function CanalMetasDialog({ open, onOpenChange, ano }: CanalMetasDialogProps) {
  const { user } = useAuth();
  const { data: metas } = useCanalMetas(ano);
  const upsert = useUpsertCanalMeta();
  const [mes, setMes] = useState(() => new Date().getMonth() + 1);
  const [vals, setVals] = useState<{ consumo: string; revenda: string; online: string }>({
    consumo: "", revenda: "", online: "",
  });
  ...
  const save = async () => {
    ...
    for (const canal of ["consumo", "revenda", "online"] as const) {
      ...
      await upsert.mutateAsync({ ano, mes, canal, valor, updatedBy: user?.id });
      n++;
    }
    if (n > 0) toast.success("Metas atualizadas!");
    onOpenChange(false);
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Defina a meta mensal de cada canal. Salva apenas os campos preenchidos.
          </DialogDescription>
        </DialogHeader>
              ...
              <SelectContent>
                {Array.from({ length: 12 }, (_, i) => i + 1).map((m) => (
                  <SelectItem key={m} value={String(m)}>
                    {format(new Date(ano, m - 1, 1), "MMMM 'de' yyyy", { locale: ptBR })}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          {CANAIS.map((c) => (
            <div key={c.key} className="space-y-1">
              <Label className="text-xs flex items-center gap-1">
                {c.label}
                <span className="text-[10px] text-muted-foreground font-normal">— {c.hint}</span>
              </Label>
