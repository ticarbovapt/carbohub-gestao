-- ─────────────────────────────────────────────────────────────────────────────
-- Carbo Chat — arquivar conversa (flag por usuário em chat_channel_members).
-- Recria chat_conversations devolvendo também "archived" (muda tipo → DROP antes).
-- ─────────────────────────────────────────────────────────────────────────────
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  SELECT ch.id, ch.type, ch.name, ch.is_private, ch.avatar_url,
         ...
         my.muted, my.pinned, my.archived
  FROM ch
