...
-- (department é enum department_type → cast ::text antes de comparar.)
-- ─────────────────────────────────────────────────────────────────────────────
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT public.carbo_is_gestor(auth.uid())
      OR EXISTS (
        ...
        WHERE p.id = auth.uid()
          AND (upper(btrim(coalesce(p.department::text, ''))) = 'RH'
            OR upper(btrim(coalesce(p.secondary_department::text, ''))) = 'RH')
      );
$$;
