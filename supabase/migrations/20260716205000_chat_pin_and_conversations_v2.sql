-- ─────────────────────────────────────────────────────────────────────────────
-- Fixar conversa (pin) por usuário + expor muted/pinned na lista de conversas.
-- ─────────────────────────────────────────────────────────────────────────────
...
RETURNS TABLE (
  ...
  unread bigint, last_activity timestamptz, muted boolean, pinned boolean
)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  WITH my AS (
    SELECT m.channel_id, m.last_read_at, m.muted, m.pinned
