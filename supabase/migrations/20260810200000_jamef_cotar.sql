-- ═══════════════════════════════════════════════════════════════════════════
-- jamef_cotar — cotação de frete pela tabela de contrato da Jamef
--
-- Fica no BANCO e não no front porque /logistica, /vender e o que vier depois
-- precisam do mesmo número. Preço divergente entre telas é pior que preço
-- ausente.
--
-- Retorna SEMPRE um jsonb; nunca levanta exceção por destino não atendido —
-- "não atendemos" é resposta legítima e a tela precisa mostrar o motivo.
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  if v_faixa.atendimento = 'R' then
    v_avisos := v_avisos || 'Atendimento por REDESPACHO — confirmar prazo e condição com a Jamef.';
  end if;
  ...
  if v_aliq is null or v_aliq <= 0 or v_aliq >= 1 then
    ...
    v_icms   := null;
    v_avisos := v_avisos || format('Sem alíquota de ICMS cadastrada para %s — total exibido SEM ICMS.', v_faixa.uf);
  else
  ...
  if coalesce(p_valor_nf, 0) <= 0 then
    v_avisos := v_avisos || 'Sem valor de NF: Ad Valorem zerado e GRIS no mínimo. Informe o valor da mercadoria para o preço fechar.';
  end if;
