-- ─────────────────────────────────────────────────────────────────────────────
-- Central de Auditoria (Admin): feed unificado de TODOS os logs do sistema.
-- Uma RPC SECURITY DEFINER que valida que o chamador é gestor e devolve um feed
-- normalizado (UNION ALL) das tabelas de log espalhadas pelo banco — cada uma tem
-- RLS diferente, então centralizar numa função definer evita telas vazias e dá um
-- único ponto sortável/filtrável para acompanhar movimentações e possíveis fraudes.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  WITH feed AS (
    ...
    UNION ALL
    -- Vendas: exclusão de pedido/venda
    ...
    UNION ALL
    -- CRM: transferência de responsável
    ...
    UNION ALL
    -- Vendas: histórico de status do pedido
    ...
    UNION ALL
    -- Vendas: auditoria de mudanças em pedidos (before/after)
    SELECT 'order_audit', 'Vendas', a.created_at, a.user_id, NULL::text,
           COALESCE(a.action, 'ação'),
           a.table_name || COALESCE(' #' || LEFT(a.record_id::text, 8), ''),
           COALESCE(a.action, 'ação') || ' em ' || a.table_name,
    ...
    UNION ALL
    -- Produção: histórico de etapas da OP/OS
    ...
    UNION ALL
    -- Financeiro/RH: auditoria de dados financeiros de funcionário
    ...
    UNION ALL
    -- Governança: bloqueios/overrides de fluxo
    ...
    UNION ALL
    -- Governança: acessos/ações sensíveis
  ...
  resolved AS (
    SELECT f.source, f.category, f.event_at, f.actor_id,
           COALESCE(NULLIF(f.actor_raw, ''), pr.full_name, pr.username, '—') AS actor_name,
