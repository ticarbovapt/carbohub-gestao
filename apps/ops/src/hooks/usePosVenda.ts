...
export interface PosVendaOrder {
  ...
  status: string | null;              // 'agendado' = parcela de recorrência ainda não devida
  nf_access_key: string | null;       // chave da NF-e — junto com invoice_number, trava a edição
  scheduled_month: string | null;     // mês devido da parcela de recorrência
  recurrence_index: number | null;    // 2 de 6
