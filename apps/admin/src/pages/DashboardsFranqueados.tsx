...
import { Building2, DollarSign, Wrench, CheckCircle2, AlertTriangle, Trophy, Gauge, Receipt, Store } from "lucide-react";
import {
...
export default function DashboardsFranqueados() {
  ...
  // Filtro de período — governa TODOS os blocos por período desta tela.
  const [range, setRange] = useState<PeriodRange>(() => presetRange("month"));

  const { data: kpis, isLoading: kLoad } = useFranqueadosKpis(range);
  const { data: months = [] } = useFranqueadosRevenueMonthly(12);
  const { data: ranking = [] } = useFranqueadosRanking(range);
      ...
      <Card className="rounded-2xl border-0 shadow-sm">
        <CardHeader className="pb-1 pt-5 px-5">
          <CardTitle className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
            Atividade Diária no Período
          </CardTitle>
          <p className="text-xs text-muted-foreground mt-0.5">Serviços e receita por dia na rede</p>
        </CardHeader>
        <CardContent className="px-2 pb-4">
          {daily.length === 0 ? (
            <p className="px-3 py-12 text-center text-sm text-muted-foreground">Sem atividade no período selecionado.</p>
          ) : (
