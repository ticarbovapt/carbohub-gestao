...
// ─────────────────────────────────────────────────────────────────────────────
// Tipo da demanda. O TI recebe muito mais coisa do que software: cabo que
// queimou, login novo, "me ensina a fazer isso". Antes tudo isso virava "bug",
// o que estragava a métrica de qualidade do sistema e escondia o volume de
// suporte de bancada. Os dois tipos originais ficaram INTACTOS (mesmo valor,
// mesma ordem) — nada foi migrado; as categorias novas só somam.
//
...
export interface KindConfig {
  key: string;        // valor gravado em bug_reports.kind
  label: string;      // rótulo visível
  hint: string;       // explicação curta pro usuário
  /** true = demanda de software (pede Sistema/app). false = não-software. */
  software: boolean;
}

export const KINDS: KindConfig[] = [
  { key: "bug",      label: "Bug",         hint: "Algo do sistema está errado",              software: true },
  { key: "sugestao", label: "Sugestão",    hint: "Ideia de melhoria no sistema",             software: true },
  // Infra: o pedido do dia a dia que não tem tela nenhuma (o exemplo real foi
  // "buscar um cabo pro monitor").
  { key: "infra",    label: "Equipamento", hint: "Cabo, monitor, periférico, máquina, rede", software: false },
  // Acesso: alto volume, resolução rápida, mede-se separado do resto. Não pede
  // Sistema porque o alvo tanto pode ser um app nosso quanto Bling, e-mail ou
  // rede — obrigar a escolher entre nossos apps só induziria resposta errada.
  { key: "acesso",   label: "Acesso",      hint: "Criar login, liberar permissão, resetar senha", software: false },
  // Ajuda: separa "não sei usar" de "está quebrado". Continua pedindo Sistema
  // porque a dúvida quase sempre é sobre uma tela nossa e isso roteia pra quem
  // conhece; dúvida sobre equipamento vai em Equipamento.
  { key: "ajuda",    label: "Ajuda",       hint: "Dúvida de uso, \"me ensina a…\"",          software: true },
];
...
/** Ícone/cor do tipo — fora do KindConfig porque é detalhe de UI e o contrato
 *  dessa struct é compartilhado com o filtro do quadro. */
