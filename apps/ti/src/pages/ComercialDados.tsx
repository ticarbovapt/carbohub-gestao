...
// Cabeçalho ordenável. Genérico em <C> para servir às duas abas — a de
// Clientes e a de Pedidos ordenam por colunas diferentes.
function SortTh<C extends string>({ col, label, sort, onSort, align = "left" }: {
  col: C; label: string; sort: { col: C; dir: "asc" | "desc" }; onSort: (c: C) => void; align?: "left" | "center" | "right";
}) {
...
export default function ComercialDados() {
  ...
  // Aba Pedidos: começa pelo mais recente, que é como a tela já vinha.
  const [pedSort, setPedSort] = useState<{ col: PedCol; dir: "asc" | "desc" }>({ col: "data", dir: "desc" });
  ...
  const togglePedSort = (col: PedCol) =>
    setPedSort((s) => (s.col === col
      ...
      // Texto começa A→Z; número e data começam do maior/mais recente, que é
      // o que se quer ver primeiro em quase todo caso.
      : { col, dir: (col === "total" || col === "data" || col === "conta") ? "desc" : "asc" }));

  const rowsOrdenadas = useMemo(() => {
    const dir = pedSort.dir === "asc" ? 1 : -1;
    const val = (o: ComercialOrderRow): string | number => {
      switch (pedSort.col) {
        case "pedido":   return (o.order_number ?? "").toLowerCase();
        case "data":     return o.created_at ?? "";
        case "cliente":  return (o.customer_name ?? "").toLowerCase();
        case "vendedor": return (o.vendedor_name ?? "zzz").toLowerCase();
        // Sem canal vai para o fim em A→Z, senão os "não classificado"
        // ocupariam o topo e escondem o que interessa.
        case "canal":    return o.segmento ? (CANAL_LABEL[o.segmento] ?? o.segmento).toLowerCase() : "zzz";
        case "status":   return (STATUS_LABEL[o.status ?? ""] ?? o.status ?? "zzz").toLowerCase();
        case "total":    return o.total ?? 0;
        case "conta":    return o.contaMetrica ? 1 : 0;
        case "origem":   return origemLabel(o).toLowerCase();
      }
    };
  ...
  const visibleIds = useMemo(() => rowsOrdenadas.slice(0, 500).map((o) => o.id), [rowsOrdenadas]);
  ...
  const exportCsv = () => {
    ...
    // Exporta na ordem que está na tela — CSV em ordem diferente da
    // exibida faz quem conferiu na tela conferir de novo no Excel.
    const lines = rowsOrdenadas.map((o: ComercialOrderRow) => [
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-board-surface">
                  <tr className="border-b text-left text-xs text-muted-foreground">
                    ...
                    <SortTh col="pedido"   label="Pedido"   sort={pedSort} onSort={togglePedSort} />
                    <SortTh col="data"     label="Data"     sort={pedSort} onSort={togglePedSort} />
                    <SortTh col="cliente"  label="Cliente"  sort={pedSort} onSort={togglePedSort} />
                    <SortTh col="vendedor" label="Vendedor" sort={pedSort} onSort={togglePedSort} />
                    <SortTh col="canal"    label="Canal"    sort={pedSort} onSort={togglePedSort} />
                    <SortTh col="status"   label="Status"   sort={pedSort} onSort={togglePedSort} />
                    <SortTh col="total"    label="Total"    sort={pedSort} onSort={togglePedSort} align="right" />
                    <SortTh col="conta"    label="Conta?"   sort={pedSort} onSort={togglePedSort} align="center" />
                    <SortTh col="origem"   label="Origem"   sort={pedSort} onSort={togglePedSort} />
                    <th className="px-3 py-2 font-medium text-center">Editar</th>
                  </tr>
                </thead>
                <tbody>
                  {rowsOrdenadas.slice(0, 500).map((o) => (
