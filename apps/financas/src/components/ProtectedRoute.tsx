...
export function ProtectedRoute({ children }: { children: ReactNode }) {
  ...
  // liberou o Finanças. O `!profile` está EXPLÍCITO aqui, e não só dentro do
      ...
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-background px-6 text-center">
        ...
        <div>
          <h1 className="text-xl font-semibold">
            {profile ? "Acesso ao Finanças não liberado" : "Conta sem cadastro interno"}
          </h1>
          <p className="text-sm text-muted-foreground mt-1 max-w-sm">
            {profile
              ? "Seu perfil não tem o Carbo Finanças habilitado. Solicite a liberação ao time de gestão (feita no Carbo Admin)."
