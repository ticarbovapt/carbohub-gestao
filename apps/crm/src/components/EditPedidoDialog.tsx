...
type OrderType = "spot" | "recorrente";
...
// Status crm_vendas → status do form (igual ao Controle).
function mapVendaStatus(s: string | null | undefined): OrderStatus {
...
function useLicensees() {
  return useQuery({
    queryKey: ["crm_edit_licensees"],
    queryFn: async (): Promise<LicenseeRow[]> => {
...
// Lê um campo de string de um endereço jsonb.
function ja(e: Record<string, unknown> | null | undefined, k: string): string {
  if (!e || e[k] == null) return "";
  return String(e[k]);
}
...
interface EditPedidoDialogProps {
  ...
  /** Trata como gestor por padrão (mostra Data real da venda e Vendedor). */
...
export function EditPedidoDialog({ vendaId, open, onOpenChange, canEditSensitive = true }: EditPedidoDialogProps) {
  ...
  // Prefill a partir da venda (crm_vendas) quando os dados chegam.
  useEffect(() => {
    if (venda) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[calc(100%-2rem)] sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Atualize as informações do pedido {orderNumber}
          </DialogDescription>
        </DialogHeader>
                                ...
                                <FormControl>
                                  ...
                                  >
                                    ...
                                    {field.value
                                      ...
                                      : `Padrão: ${venda?.created_at ? format(parseISO(venda.created_at), "dd/MM/yyyy") : "—"}`}
                                  </CarboButton>
                                </FormControl>
                              </PopoverTrigger>
              ...
              <TabsContent value="po" className="space-y-5 mt-4">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-[11px] text-muted-foreground -mt-1">
                    Vincule esta venda à NF emitida no Bling. A chave de acesso é preenchida automaticamente quando o financeiro importa a NF com o número <span className="font-mono font-medium">{orderNumber}</span> na observação.
                  </p>
                  <div className="grid grid-cols-2 gap-4">
                    <FormField
                      control={form.control}
